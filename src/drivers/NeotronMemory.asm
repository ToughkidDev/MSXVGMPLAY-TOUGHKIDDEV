...
NeotronMemory_Construct:
	...
	ld bc,63
	ldir
	; The openMSXTK NeoTron extension exposes the SIOS entry points as RET
	; stubs.  Such a stub has no defined carry result, unlike real SIOS.  Mark
	; that one explicit compatibility case once, rather than masking a genuine
	; hardware error after every sample-memory write.
	xor a
	ld (ix + NeotronMemory.statusUndefined),a
	ld a,(ix + NeotronMemory.slot)
	ld hl,SIOS_WRITE_SMEM
...
NeotronMemory_Write:
	...
	pop iy
	pop ix
	bit 0,(ix + NeotronMemory.statusUndefined)
	ret z
	; The emulator's plain RET stub leaves carry inherited from the caller.
	; Clear it only for that detected stub; real SIOS errors remain visible.
	and a
	ret
