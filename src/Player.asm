...
; ix = this
; iy = reader
; Works for both the post-scan MappedReader and the StreamFileReader used
; while a plain VGM is being compacted.  The generic Reader routine consumes
; bytes sequentially, so no reader-specific address calculation is needed.
Player_SkipDataBlock:
	push de
	ld b,h
	ld c,l
	pop de
...
Player_EndOfSoundData: PROC
	...
	pop ix
	jr z,NoLoop
	; Header lookup and scanner callbacks do not promise to retain IY.  The
	; loop target is a 32-bit value, so preserve it while reacquiring the
	; player-owned mapped reader before using the MappedReader methods below.
	push de
	push hl
	push ix
	call Player_GetReader_IY
	...
	pop hl
	pop de
	; A streamed command buffer translates the original VGM loop address while
	; PCM blocks are removed.  Treat an invalid translated loop target as no
	; loop instead of allowing MappedReader_SetPosition_IY to abort playback.
	; This also protects malformed VGM loop offsets in the same way as the
	; stream loader's "loop point not found" fallback.
	push ix
	...
	; Selecting a mapper segment may pass through BIOS mapper helpers.  Reload
	; the player-owned reader explicitly before Scanner resumes the loop.
	push ix
	call Player_GetReader_IY
	pop ix
